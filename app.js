...
function loadDraft() {
  const raw = localStorage.getItem("print_draft_v3");
...
function saveDraft() {
  localStorage.setItem("print_draft_v3", JSON.stringify(state.draft));
}

function loadUndoStack() {
  const raw = localStorage.getItem("print_undo_stack_v3");
  if (!raw) return [];
  try {
    return JSON.parse(raw);
  } catch {
...
function saveUndoStack() {
  localStorage.setItem("print_undo_stack_v3", JSON.stringify(state.undoStack.slice(-25)));
}
...
function formatDisplayValueFromRaw(raw) {
  const number = parseEuroInputString(raw);
  return euro(number);
}
...
async function deleteFavoriteColorFromDb(id) {
  ...
  if (error) {
    console.error("Favorit konnte nicht gelöscht werden:", error);
...
async function saveFavoriteColorToDb(name, hex) {
  ...
  if (!cleanName) {
    alert("Bitte gib zuerst einen Namen für die Farbe ein.");
    return false;
  }
...
function getCurrentProductConfig() {
  ...
  if (product === "rabbit") {
    return {
      ...
      colors: [
        { key: "ears", label: "Ohren", editableLabel: true },
        { key: "body", label: "Körper", editableLabel: true },
        { key: "tail", label: "Popo", editableLabel: true }
      ],
      verticalColors: true,
  ...
  if (product === "lumi") {
    return {
      sizeRequired: false,
      colors: [
        { key: "body", label: "Körperfarbe", editableLabel: true }
      ],
  ...
  if (product === "text") {
    return {
      ...
      sizeLabel: "Größe",
      colors: [
        { key: "c1", label: "Erste Farbe", editableLabel: true },
        { key: "c2", label: "Zweite Farbe", editableLabel: true }
      ],
...
function renderProductForm() {
  ...
  renderColorFields(config.colors);
  ...
  const addExtraColorBtn = document.getElementById("addExtraColorBtn");
  if (addExtraColorBtn) {
    addExtraColorBtn.addEventListener("click", handleAddExtraColor);
  }
...
function renderColorFields(colors) {
  ...
  wrap.innerHTML = colors.map((color) => {
    const labelText = color.label || "Farbe";
    ...
    return `
      <button
        ...
        data-color-label="${escapeHtml(labelText)}"
        data-color-hex="${hex}"
      >
        <span class="color-chip-label">
          <span class="color-dot" style="background:${hex};"></span>
          <span class="color-text">${escapeHtml(labelText)}</span>
        </span>
        <span>Ändern</span>
      </button>
    `;
...
function openColorModal(targetBtn) {
  ...
  const label = targetBtn.dataset.colorLabel || "";
  const hex = targetBtn.dataset.colorHex || "#d4af37";

  els.colorNameInput.value = label;
...
async function saveStockEntry() {
  try {
    ...
    const { data: insertedItems, error: itemError } = await supabaseClient
      .from("order_items")
      .insert(itemPayload)
      .select();
...
function renderColorListText(colors) {
  ...
  return colors.map((c) => escapeHtml(c.label || "Farbe")).join(", ");
}
...
async function restoreDeletedOrder(orderSnapshot, itemsSnapshot) {
  const { data: restoredOrder, error: orderError } = await supabaseClient
    .from("orders")
    .insert({
      id: orderSnapshot.id,
  ...
  showConfirm(`Wirklich rückgängig machen: ${last.label}?`, async () => {
    ...
    if (action.type === "restore_total_price") {
      await supabaseClient
        .from("orders")
        .update({ total_price: action.previousTotal })
...
function bindEvents() {
  ...
  els.colorNameInput.addEventListener("input", () => {
    // nur Bezeichnungsfeld für die aktuelle Produktfarbe
  });

   els.applyColorBtn.addEventListener("click", () => {
    if (!state.colorModalTarget) return;

    const label = els.colorNameInput.value.trim() || "Farbe";
    const hex = els.customColorPicker.value;

    state.colorModalTarget.dataset.colorLabel = label;
    state.colorModalTarget.dataset.colorHex = hex;
    state.colorModalTarget.innerHTML = `
      <span class="color-chip-label">
        <span class="color-dot" style="background:${hex};"></span>
        <span class="color-text">${escapeHtml(label)}</span>
      </span>
      <span>Ändern</span>
    `;
  ...
  els.closeColorDialog.addEventListener("click", () => els.colorDialog.close());
  ...
  els.closePriceDialog.addEventListener("click", () => els.priceDialog.close());

  els.closeConfirmDialog.addEventListener("click", () => els.confirmDialog.close());
