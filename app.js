...
function loadDraft() {
  const raw = localStorage.getItem("print_draft_v4");
...
function saveDraft() {
  localStorage.setItem("print_draft_v4", JSON.stringify(state.draft));
}

function loadUndoStack() {
  const raw = localStorage.getItem("print_undo_stack_v4");
  if (!raw) return [];
  try {
    return JSON.parse(raw);
  } catch {
...
function saveUndoStack() {
  localStorage.setItem("print_undo_stack_v4", JSON.stringify(state.undoStack.slice(-25)));
}
...
function getCurrentProductConfig() {
  ...
  if (product === "lumi") {
    return {
      sizeRequired: false,
      colors: [{ key: "body", label: "Körperfarbe" }],
